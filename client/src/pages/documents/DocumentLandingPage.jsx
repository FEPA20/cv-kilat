...
function getDocumentStatus(document) {
  return String(document?.status || "COMING_SOON").toUpperCase();
}

function isDocumentActive(document) {
...
function StatusBadge({ status, className = "" }) {
  ...
  const meta =
    DOCUMENT_STATUS_META[normalizedStatus] ||
    DOCUMENT_STATUS_META.COMING_SOON;

  return (
    <span
      className={
        "inline-flex items-center rounded-full px-3 py-1 text-[11px] font-black uppercase tracking-[0.12em] ring-1 " +
        meta.className +
        " " +
        className
      }
        ...
        <div className="mt-6 grid gap-3 sm:grid-cols-[1fr_auto]">
          {isDocumentActive(document) ? (
            ...
            <button
              ...
              title="Dokumen ini masih berstatus COMING_SOON"
            >
